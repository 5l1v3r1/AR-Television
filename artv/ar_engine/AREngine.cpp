...
namespace ar {
    ...
    void AREngine::UpdateInterestPoints(const cv::Mat &scene) {
        ...
        if (frame_id_) {
            // Perform matching.
            cv::Mat stored_descriptors(static_cast<int>(interest_points_.size()), desc_length_, CV_32F);
            for (int i = 0; i < interest_points_.size(); ++i)
        ...
        for (int i = 0; i < keypoints.size(); ++i) {
            if (!matched_new[i]) {
                interest_points_.push_back(std::make_shared<InterestPoint>(
                        frame_id_, keypoints[i], descriptors.row(i)));
            }
    ...
    ERROR_CODE AREngine::FeedScene(const Mat &raw_scene) {
        ...
        if (keyframe_seq_tail_ == -1) {
            ...
            auto average_depth = sum(transformed_pts3d.col(2))[0];
